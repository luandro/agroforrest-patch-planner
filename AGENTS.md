...
## Testing Guidelines
Vitest + Testing Library are not wired; add them alongside critical features and co-locate specs (`ComponentName.test.tsx`) while PRs still document manual QA. Persistence-heavy work must be browser-tested because IndexedDB mocks are absent—see `./context/05-state-and-storage.md` for pitfalls.
